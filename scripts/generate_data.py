...
INDUSTRIES = ["Technology", "Financial Services", "Healthcare", "Manufacturing",
              "Retail", "Energy", "Government", "Education", "Media", "Telecommunications"]
CAMPAIGN_TYPES = ["Webinar", "Event", "Content Syndication", "Email", "Advertisement", "Telemarketing", "Direct Mail"]
MEMBER_STATUSES = {"Webinar": ["Registered", "Attended", "No Show"],
...
LEAD_SOURCES = ["Web", "Event", "Content Syndication", "Purchased List", "Partner Referral", "Organic Search"]
JOB_PERSONAS = ["Technical Buyer", "CISO", "Financial Buyer", "IT Operations", "Security Engineer", None]
...
CYBERSECURITY_COMPANIES = ["CrowdStrike", "Palo Alto Networks", "Fortinet", "SentinelOne",
...
def generate_accounts():
    ...
    for i in range(NUM_ACCOUNTS):
        ...
        accounts.append({
            ...
            "annual_revenue": round(random.choice([1e6, 5e6, 10e6, 50e6, 100e6, 500e6, 1e9]), 0),
            "is_icp_qualified": is_icp,
            "is_named_account": is_named,
            "intent_score": intent,
            "do_not_contact": random.random() < 0.03,
            "country": random.choice(["US", "US", "US", "UK", "Canada", "Germany", "Australia"]),
        })
...
def generate_leads(accounts_df):
    ...
    for i in range(NUM_LEADS):
        ...
        lead_status = "Disqualified" if is_disqualified else ("MQL" if is_mql else random.choice(["New", "Attempted", "Qualified", "Recycled"]))
...
def generate_contacts(accounts_df, leads_df):
    ...
    for i in range(NUM_CONTACTS):
        ...
        contacts.append({
            ...
            "last_name": fake.last_name(),
            "title": title,
            "contact_status": "MQL" if is_mql else random.choice(["Active", "Attempted", "Qualified", "Nurture"]),
            "job_persona": persona,
            "job_level": level,
            "created_date": created.strftime("%Y-%m-%d"),
            "mql_date": mql_date.strftime("%Y-%m-%d") if mql_date else None,
            ...
            "primary_lead_id": f"LEAD-{random.randint(1, NUM_LEADS):04d}" if has_lead_origin else None,
...
def inject_personas(leads_df, contacts_df, accounts_df, cm_df):
    ...
    leads_df.loc[0] = {**leads_df.loc[0],
        "title": "VP of Security", "job_persona": "CISO", "job_level": "VP",
        "lead_status": "MQL", "mkto_lead_score": 85, "is_converted": False,
    ...
    if named_accts:
        contacts_df.loc[0] = {**contacts_df.loc[0],
            "account_id": named_accts[0], "title": "VP of Security",
            "job_persona": "CISO", "job_level": "VP", "is_mql": True,
            "mkto_contact_score": 90,
            "mql_date": (NOW - timedelta(days=5)).strftime("%Y-%m-%d")}

    # Persona 2: Same profile but stale engagement (>6 months old) - use index 1
    leads_df.loc[1] = {**leads_df.loc[1],
        "title": "VP of Information Security", "job_persona": "CISO", "job_level": "VP",
        "lead_status": "Recycled", "mkto_lead_score": 65, "is_converted": False,
        "mql_date": (NOW - timedelta(days=200)).strftime("%Y-%m-%d")}

    # Persona 3: Junior analyst, 15 responses in 30 days, not MQL
    leads_df.loc[2] = {**leads_df.loc[2],
        "title": "Security Analyst", "job_persona": "Security Engineer",
        "job_level": "Individual Contributor", "lead_status": "New",
        "mkto_lead_score": 35, "mql_date": None, "company": "TinyStartup Inc"}

    # Persona 4: CISO Fortune 500, zero engagement
    leads_df.loc[3] = {**leads_df.loc[3],
        "title": "Chief Information Security Officer", "job_persona": "CISO",
        "job_level": "C-Level", "lead_status": "New", "mkto_lead_score": 5,
        "lead_source": "Purchased List", "mql_date": None}

    # Persona 5: Competitor employee
    leads_df.loc[4] = {**leads_df.loc[4],
        "title": "Solutions Engineer", "job_persona": "Non-Prospect",
        "job_level": "Individual Contributor", "company": "CrowdStrike",
        "lead_status": "New", "mkto_lead_score": 70}

    # Persona 6: Bounced + opted out but recent physical event
    leads_df.loc[5] = {**leads_df.loc[5],
        "has_opted_out": True, "email_bounced": True,
        "lead_status": "Attempted", "mkto_lead_score": 45}

    # Persona 7: Broken conversion link (DQ-1)
    leads_df.loc[6] = {**leads_df.loc[6],
        "is_converted": True, "converted_contact_id": None,
        "lead_status": "MQL", "mkto_lead_score": 60}

    # Persona 8: 40 campaign memberships, 38 automated emails
    leads_df.loc[7] = {**leads_df.loc[7],
        "title": "IT Manager", "lead_status": "MQL", "mkto_lead_score": 55}

    # Persona 9: Re-MQL'd 4 times
    leads_df.loc[8] = {**leads_df.loc[8],
        "lead_status": "MQL", "mkto_lead_score": 50, "is_disqualified": False,
    ...
    if named_accts and len(named_accts) > 1:
        contacts_df.loc[1] = {**contacts_df.loc[1],
            "account_id": named_accts[1], "has_lead_origin": False,
            "title": "Director of IT", "job_persona": "Technical Buyer",
            "job_level": "Director", "mkto_contact_score": 40}
